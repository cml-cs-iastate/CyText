...
def get_interpretation(interpretation_method, input_model, input_x, input_y, window_size, target_layer_name):
    ...
    """
    predicted_features = []
    for idx, window_size in enumerate(window_size):
...
# Tested
def get_interpretation_de(input_model_path, xs, y_train, num_classes, interpretation_method='grad*input'):
    """
    ...
    with DeepExplain(session=tf.compat.v1.keras.backend.get_session()) as de:
        ...
        ys = np.zeros(shape=(len(y_train), num_classes))
        for i, label in enumerate(y_train):
            ys[i, label] = 1

        y_prediction = new_model.predict(xs)
        ...
        embedding_output = embedding_function([xs])
        ...
        return interpretation_heatmap, y_prediction
