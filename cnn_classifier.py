...
class cnn_clf(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope('interpretation'):
            ...
            if self.lrp:
                ...
                if self.salience_map:
                    ...
                    heat_map_2 = self.run_salience_map(h_2_gradients, 2, 'salience_map_2')  # shape=(?, 250)
                    heat_map_3 = self.run_salience_map(h_3_gradients, 3, 'salience_map_3')  # shape=(?, 250)
                    heat_map_4 = self.run_salience_map(h_4_gradients, 4, 'salience_map_4')  # shape=(?, 250)
                else:
                    # generate the heat map for each convolutional layer
                    heat_map_2 = self.run_selfie(h_2_gradients, convolution_layer_2[2], convolution_layer_2[3], 2, 'heat_map_2')  # shape=(?, 250)
                    heat_map_3 = self.run_selfie(h_3_gradients, convolution_layer_3[2], convolution_layer_3[3], 3, 'heat_map_3')  # shape=(?, 250)
                    heat_map_4 = self.run_selfie(h_4_gradients, convolution_layer_4[2], convolution_layer_4[3], 4, 'heat_map_4')  # shape=(?, 250)

                # combine the three convolutional layers
                # shape=(32, 250)
                heat_map = tf.nn.relu(tf.add_n([heat_map_2, heat_map_3, heat_map_4], name='heat_map'))
        ...
        with tf.name_scope('loss'):
            ...
            jaccard_predictions = tf.divide(intersection + tf.constant(1e-10), (union - intersection) + tf.constant(1e-10))
    ...
    def run_salience_map(self, h_gradients, filter_size, tensor_name):
        """
        ...
        """
        # shape = (32, 250)
        z = tf.concat([tf.reduce_sum(tf.squeeze(h_gradients, [2]), axis=2), tf.zeros([self.batch_size, filter_size - 1])], axis=1, name=tensor_name)
        return z

    def run_selfie(self, h_gradients, w_, b_, filter_size, tensor_name):
        """
        generate the class activation map
        :param h_gradients: the gradient to the convolutoinal layer
        :param w_: the kernel of filter size filter_size
        :param b_: the kernal bias
        :param filter_size: the filter window sizes
        :param tensor_name: the name of the output tensor
        :return:
        """
        # shape = (32, 250, 1, 128) [batch_size, max_length-filter_size+1, 1, num_filters]
        z = tf.concat([h_gradients, tf.zeros([self.batch_size, filter_size - 1, 1, self.num_filters])], axis=1)
