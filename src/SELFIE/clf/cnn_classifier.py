...
class cnn_clf(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope('embedding'):
            ...
            # shape = [batch_size, max_length, embedding_size, 1]
        ...
        with tf.name_scope('interpretation'):
            ...
            if self.lrp:
                ...
                if self.salience_map:
                    ...
                    heat_map_2 = self.run_salience_map(h_2_gradients, 2) 
                    heat_map_3 = self.run_salience_map(h_3_gradients, 3) 
                    heat_map_4 = self.run_salience_map(h_4_gradients, 4)  
                else:
    ...
    def convolution_layer(self, filter_size):
        ...
        with tf.compat.v1.variable_scope('convolution_layer_%s' % filter_size):
            ...
            # max-pooling
            pooled = tf.nn.max_pool(h, ksize=[1, self.max_length - filter_size + 1, 1, 1], strides=[1, 1, 1, 1], padding='VALID', name='pool')
            return [h, pooled, w, b]
    ...
    def run_lrp(self, softmax_w_, softmax_b_, h_drop_, target_logits_, num_filters_total_, convolution_layer_2_, convolution_layer_3_, convolution_layer_4_):
        ...
        for i in range(self.batch_size):
            ...
            softmax_weights_ = softmax_w_[:, self.predictions[i]]       
            softmax_bias_ = softmax_b_[self.predictions[i]] 
            ...
            inputs_for_lrp_2 = list()
            inputs_for_lrp_3 = list()
            inputs_for_lrp_4 = list()
            ...
            inputs_for_lrp_2 = tf.concat(inputs_for_lrp_2, axis=2)
            inputs_for_lrp_3 = tf.concat(inputs_for_lrp_3, axis=2)
            inputs_for_lrp_4 = tf.concat(inputs_for_lrp_4, axis=2)
